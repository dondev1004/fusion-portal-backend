...
exports.userCreate = async (req, res) => {
    ...
    const groups = await prisma.view_groups.findMany({ where: { group_level: { lt: userRole.ADMIN + 1 } } });

    const defaultDomain = await prisma.v_domains.findFirst({ where: { domain_name: process.env.DEFAULT_DOMAIN } });
    
    if (req.method == 'GET') {

        return res.status(resCode.SUCCESS).json({ msg: resMessage.SUCCESS, data: groups });

    } else if (req.method == 'POST') {
        try {
            const { username, firstName, lastName, password, email, organization, group = '5d0f9ba9-f9cd-4ff1-9004-1021139cb99e' } = req.body;
            ...
            if (user) return res.status(resCode.ALREADY_EXIST).json({ msg: resMessage.ALREADY_EXIST });
            ...
            const newContact = await prisma.v_contacts.create({
                data: {
                    ...
                    contact_name_family: firstName ? firstName : '',
                    contact_name_given: lastName ? lastName : '',
                    contact_organization: organization ? organization : '',
            ...
            if (!isEmpty(group)) {
                ...
                const userRegister = await prisma.v_user_groups.create({
                    data: {
                ...
                if(isEmpty(userRegister)) return res.status(resCode.NO_EXIST).json({ msg: resMessage.CREATE_FAILED });
            }
...
exports.userUpdate = async (req, res) => {
    ...
    if (req.method == 'GET') {
        ...
        return res.status(resCode.SUCCESS).json({ msg: resMessage.SUCCESS, data: { groups, user } });

    } else if (req.method == 'PUT') {
        try {
            const { username, firstName, lastName, password, email, organization, group } = req.body;
            const admin = req.user;
...
exports.userDelete = async (req, res) => {
    try {
        ...
        if (isEmpty(deleteUser)) return res.status(resCode.NO_EXIST).json({ msg: resMessage.DELETE_FAILED });

        return res.status(resCode.SUCCESS).json({ msg: resMessage.DELETED });
    } catch (err) {
        console.log(err);
        return res.status(resCode.SERVER_ERROR).json({ msg: err });
    }
    ...
    try {
        if (isEmpty(query)) {
            domains = await prisma.v_domains.findMany({
                where: { domain_enabled: true }, skip, take,
            });
    ...
    try {
        ...
        const newDomain = await prisma.v_domains.create({
            data: {
                ...
                domain_description: domain_description ? domain_description : '',
                domain_enabled: true,
